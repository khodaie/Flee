...
namespace Flee.InternalTypes
{
	...
	internal static class Utility
	{
		...
		internal static IList<MemberInfo> GetAllDefaultMembers(this Type type)
		{
			IList<MemberInfo> members = type.GetDefaultMembers();
			if (!type.IsInterface)
				return members;
			...
			var listMembers = members.ToList();

			foreach (var baseInterfaceType in baseInterfaceTypes)
			{
				listMembers.AddRange(baseInterfaceType.GetDefaultMembers());
			}
		...
		public static MethodInfo GetSimpleOverloadedOperator(string name, Type sourceType, Type destType)
		{
			...
			do
			{
				members = sourceType.FindAllMembers(MemberTypes.Method, flags, SimpleOverloadedOperatorFilter, data);
			} while (members.Count == 0 && (sourceType = sourceType.BaseType) != null);

			if (members.Count == 0 && destType != null)
			{
				// Look on the dest type and its ancestors
				do
				{
					members = destType.FindAllMembers(MemberTypes.Method, flags, SimpleOverloadedOperatorFilter, data);
				} while (members.Count == 0 && (destType = destType.BaseType) != null);
			}

			Debug.Assert(members.Count < 2, "Multiple overloaded operators found");
